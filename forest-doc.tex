%% forest-doc.tex
%%
%% This file is a part of `forest', a `pgf/tikz'-based LaTeX package for drawing (linguistic) trees.
%% See forest.dtx for the list of all files comprising the package.
%%
...
For example, a phonologist can use the \index{GP1} style from \S\reflibrary{linguistics} to easily
...
Furtunately, designing styles is not very difficult once you know get the hang of \foRest;, if you
...
sections~\ref{ref:spatial-propagators} and \ref{ref:node-walk}.)
...
\begin{syntax}
\indexitem{macro>useforestlibrary}|{|\meta{library name}|}|  Loads the library \meta{library name}.
\indexitem{macro>forestlibraryloaddefaults}
\end{syntax}
...
or use macro \cmdname{forestlibraryloaddefaults} like below, you get the following:
...
\begin{forestexample}[tree bin=minipage,index={for step,c-commanded}]
  ...
  ~\forestloadlibrarydefaults{linguistics}~
  \begin{forest}
    [VP
...
|compat=|\indexdef{option of=compat>all}; to load those most like to be useful, write
|compat=|\indexdef{option of=compat>most} or simply |compat|.  To load all/most compatibility
features since version |x.y|, write |x.y-all| or |x.y-most|.
...
\begin{forestexample}
    ...
    ~\Forest*~{
      [VP,baseline
        [V]
        [,replace by=DP,
          for name={DP}{for tree={s=0}},%workaround!
        ]
      ]
    }
\end{forestexample}}
...
nodes are processed (recursively, in a depth-first, parent-first fashion, unless this is changed by
\index{processing order}).  The default preamble and preamble of the current tree are processed
first, in  
...
The node whose keylist is being processed is called the \emph{current node}.  During the processing of
the keylist, the current node can temporarily change.  This mainly happens when spatial propagators
(\S\ref{ref:spatial-propagators}) are being processed.

Options are set using \PGF;'s key management utility |pgfkeys| \citep[\S55]{tikzpgf2.10}, which
introduces the concept of \emph{paths}.  The keys described in this section belong to \foRest;'s
\emph{main keypath} and are accessible even in nodewalks (see \S\ref{ref:node-walk}).  If the
given key is not found, it is appended to keylist \index{node options}, whose keys are eventually
evaluated as options of \TikZ;'s |\node| command, unless this behaviour is changed using
\index{unknown to}.

Options can be set and modified in various ways, depending on the option type (the types are listed
below).  The most straightforward way is to use the key with the same name as the option, which will
simply set the current node's value to the given value, but it is also possible to set a non-current
option.

\begin{syntax}
\indexitem[index key={@@@},form={}]{prefix>assignment>current node=\meta{option}}|=|\meta{value} Sets the value of \meta{option} of the current node to
  \meta{value}.

  Note that option types \index{keylist} and \index{autowrapped toks} redefine this basic key.

\indexitem[index key={@@@.},form={.},+print format={+result={\index(not print){relative node name}\index(not index)[+print format=definition]{relative node name}}},+index format={+result={\meta{relative node name}}}]{prefix>assignment>relative node=\meta{option}}|=|\meta{value}  Sets the value of
  ...
  \item If the \meta{relative node name} resolves to the invalid node, no assignment happens;
    \meta{value} is never evaluated.
  \item \meta{option} can also be an ``augmented assignment operator'' (see below) or, indeed, any
    key from the main keypath.
  \end{enumerate*}
\end{syntax}

We continue by listing option types and additional option-setting keys they define for each option.
(Each option type also defines several conditionals for each option. Most notably, \index{if
  option=\meta{option}} and \index{where option=\meta{option}} are defined for every \meta{option},
irrespectively of its type. For details, see \S\ref{ref:conditionals}.)
...
text} \citep[275]{texbook}. 
  ...
  \indexdef{prefix>augmented assignment=autowrapped toks>+=\meta{option}}|=|\meta{toks} are equivalent to
...
\indexitem{type>dimen} contains a dimension.
  ...
  {\rstyle\meta{option}}|=|\meta{pgfmath} is an implicit
  \meta{option}|/|\index{pgfmath}|=|\meta{pgfmath}.
...
Several mechanisms exist for reading option values:
\begin{itemize}
\item Using macros \indexdef{macro>forestoption}|{|\meta{option}|}| and
  \indexdef{macro>foresteoption}|{|\meta{option}|}|, options of the current node can be accessed in
  \TeX\ code.  Within \foRest;, this includes arguments to \index{TeX}, \PGF;'s handler |.expanded|
  and values given to \meta{toks} and \meta{keylist} options.
...
\item Using handler \index{handler>option}.
\end{itemize}

The following subsections are a complete reference of the keys residing in \foRest;'s main keypath.
Most of these keys simply set or modify options, but we will also meet several more exotic keys like
spatial (\S\ref{ref:spatial-propagators}) and temporal (\S\ref{ref:temporal-propagators})
propagators, keys that dynamically create and move nodes (\S\ref{ref:dynamic}), etc.  Before listing
all these keys, however, it is worth mentioning that users can also define their own keys.

The easiest way to do this is by using \emph{styles}.  Styles are a feature of the |pgfkeys|
package.  They are named keylists, whose usage ranges from mere abbreviations through templates to
devices implementing recursion.  To define a style, use \PGF;'s handler \keyname{.style}
\citep[\S55.4.4]{tikzpgf2.10}: \meta{style name}|/.style=|\meta{keylist}.

Using the following keys, users can also declare their own options.  The new options
will behave exactly like the predefined ones.
...
The style definitions and option declarations given among the other keys in the bracket
specification are local to the current tree.  To define globally accessible styles and options
(well, definitions are always local to the current \TeX\ group), use macro |\forestset| outside the
\index{forest} environment:\footnote{\index{forestset}\meta{keylist} is currently equivalent to
  \cmdname{pgfkeys}\texttt{\{/forest,}\meta{keylist}\texttt{\}}, but don't rely on this as it will
  change in some (near) future version of the package. There is a plan to introduce namespaces \dots}
\begin{syntax}
...
\indexitem{option>node format}|=|\meta{toks}
  ...
  \indexitem({{{{{}}}}}){option>node options}|=|\meta{keylist}
...
\index{pack}, which can be useful for manual corrections, like below.  (B is closer to A than C
because packing proceeds from the first to the last child --- the position of B would be the same
if there was no C.)  Changing the value of \keyname{s} after stage \index{compute xy} has no
effect.
...
this prefix and the exceptions.  For the list of nodewalk steps, see \S\ref{ref:node-walk}.

Spatial propagation does not change the current node: after visiting the nodes the options are
...
\indexitem{word prefix=step>propagator>for=\meta{step}}|=|\meta{arg$_1$}|...|\meta{arg$_n$}\meta{keylist}
  ...
  \meta{step} must be a long-form nodewalk step.  If it has any arguments, they should be
  given before the keylist.

  For details on nodewalk steps, see \S\ref{ref:node-walk}.
  
  Examples: |for_parent={l_sep+=3mm}|, |for_n=2{circle,draw}|, |for_nodewalk={uu2}{blue}|,
  |for_tree={s_sep+=1em}|.
...
\indexitem{node key>Nodewalk}|=|\meta{keylist: config}\meta{nodewalk}\meta{keylist: every-step}
  ...
  Configures and executes the \meta{nodewalk}.  This key is a copy of \index{step>Nodewalk}.

  \begin{advise}
  \item Use this key carefully as it can change the current node!
  \item The envisioned purpose of this key is to use it inside the every-step keylist of (an outer)
    nodewalk, where only node keys are accepted.  The config defaults (independent every-step,
    shared history) are set to facilitate that purpose.
  \end{advise}
\indexitem{node key>node walk}|=|\meta{node walk} \textbf{Deprecated!!!} This key will be removed in v2.0,
  please use \index{for step=\index{nodewalk}} in new code.  From the old documentation:
...
There are two kinds of temporal propagators.  The |before_...| and |after_...| propagators defer the
processing of the given keys to a hook just before or after some stage in the computation.  The
\keyname{delay} propagator is ``internal'' to the current hook (the first hook, the \keyname{given
  options}, is implicit): the keys in a hook are processed cyclically, and \keyname{delay} delays
the processing of the given options until the next cycle.  All these keys can be nested without
limit. For details on stages in the processing of the tree, see~\S\ref{ref:stages}.
\begin{syntax}
...
\indexitem{propagator>given options}
  ...
  When \index{stages} processing starts, this list holds the options given by the user in the
...
\item Examples: title page (in style |random tree|) and \ref{ex:ex:niceemptynodes} (in style
  |xlist|).
\end{advise}
...
Steps~\ref{step:parsing-bracket} collects user input and is thus ``fixed''.  However, the other
steps, which do the actual work, are under user's control.
...
\index{y} can only be done after the absolute positions have been computed, so the processing
of this option is deferred by \index{before drawing tree}.  For a more realistic example, see
the definition of style \index{GP1}: before packing, \texttt{outer xsep} is set to a high (user
...
preceding the execution of each but the first stage by processing the options embedded in temporal
...
Obviously, a style must be redefined before it is processed, |styles| should be redefined outside
the \index{forest} environment (using macro \index{forestset}). A stage style can also be
redefined in the preamble or in any of the keylists processed prior to entering that
stage.

Here's the list of keys used either in the default processing or useful in an alternative
processing flow.
\begin{syntax}
\indexitem{stage>typeset nodes} Typesets each node of the current node's subtree in its own
  |tikzpicture| environment.  The result is saved in a box and its measures are taken.

\indexitem{stage>typeset nodes'}  Like \index{typeset nodes}, but the node box's content is not
...
\indexitem{stage>compute xy} Computes the positions of the nodes relative to the (formal) root
  node.  The results are stored into options \index{x} and \index{y}.

\indexitem{stage>draw tree} Produces the \TikZ; code that will draw the tree.  First, the nodes
  are drawn (using the boxes typeset in step~\ref{step:typeset-nodes}), followed by edges and
  custom code (see option \index{tikz}).
  
\indexitem{stage>draw tree'} Like \index{draw tree}, but the node boxes are included in the
...
\indexitem{node key>draw tree box}|=|[\meta{\TeX\ box}]  The picture drawn by the subsequent
  ...
  \indexitem{node key>process keylist}|=|\meta{keylist option} Processes the keylist saved in
  option \meta{keylist option} for the nodewalk specified by \index{keylist option processing order=\meta{keylist option}}.  By default, this means that \emph{all the nodes in the entire tree} are processed.
  ...
  Keylist-processing proceeds in cycles.  In a given cycle, the value of option \meta{keylist option
    name} is processed for every node, in a recursive (parent-first, depth-first) fashion.  During a
  cycle, keys may be \emph{delayed} using key \index{delay}.  Keys delayed in a cycle are
  processed in the next cycle.  The number of cycles in unlimited.  When no keys are delayed in a
  cycle, the processing of a hook is finished.
...
\indexitem{node key>process keylist'}|=|\meta{keylist option}\meta{nodewalk}
  ...
  Processes the keylist saved in option \meta{keylist option} for nodes in the given
  \meta{nodewalk}.  The nodewalk starts at the current node.

  Delayed options are processed in cycles internal to the execution of this key. For details, see
  \index{process keylist}.

  With this key, there is no dynamic creation of nodes between the cycles. Any dynamic node
  ...
  It is safe to embed this key within \index{process keylist} and thus to use it in the |before_...| keylists.

  Note also that delayed options will be executed only for nodes visited in the given nodewalk.
  Delayed propageted options will be remembered, though, and executed when the given keylist is
  processed for the other node.  Using spatial propagators without delaying, or delaying options in
  the current node works as usual.
...
\indexitem{node key>process keylist''}|=|\meta{keylist option}\meta{nodewalk}
  ...
  Neither delayed options nor dynamic node operations are executed.
...
\indexitem{node key>process keylist register}|=|\meta{register}
  ...
  Any delayed options will not be processed during the execution if this key. They will be
...
\indexitem{node key>process delayed}|=|\meta{nodewalk} Process delayed options.

  Processing keylist \index{delay} using \index{process keylist} or \index{process keylist'}
  would lead to unpredictable results.  Thus this key.
  
  This key is safe to use within \index{process keylist}, \index{process keylist'} and
  \index{process keylist register}.

  (Re-)delayed options will be processed. No dynamic operations will be done: they are saved for
  later.
...
\indexitem{node key>do dynamics} Experimental. Perform pending dynamic tree operations.
  ...
  Do not use this key within \index{process keylist}, \index{process keylist'} or
  \index{process keylist register}.
...
\meta{keylist}.  This can be used to ``intercept and remember'' \TikZ; options; for example, see
\ref{sec:example-folder}.
...
This is a more generic variant of autoforwarding. Instead of adding the new value of option or
register to a keylist, a style defined by \meta{style definition} is called with the new value as
the argument.
...
\meta{separator} (which must be a single token), and process the \meta{keylist} with the pieces of
the split token list as values, in the order given.
...
Do \emph{not} use handler |.unknown| to deal with unknown keys. This handler is used internally by
\foRest;, and is set up to make it possible to set options of non-current nodes (see
\S\ref{ref:node-options}).
...
\FoRest; v1.1 introduces another concept similar to options: \emph{registers}. Unlike option values,
register values are not specific to each node. (We're trying to avoid the word global here, as
registers are not global in the \TeX nical sense of transcending groups.)  Otherwise, register function precisely as options.

Registers are set exactly the same way as node options.  The basic way is to use the register name
as a key. The same augmented assignments, depending on the type, are available, register types being
the the same as option types: \index{type>toks}, \index{type>autowrapped toks},
\index{type>keylist}, \index{type>dimen}, \index{type>count} and \index{type>boolean}.  See
\S\ref{ref:node-options} for details.

To read a register value, use macros \indexdef{macro>forestregister} and
\indexdef{macro>foresteregister} analogous to \index{forestoption} and \index{foresteoption},
handler \index{handler>register} analogous to \index{handler>option}, or |pgfmath| functions (see
\S\ref{ref:pgfmath}).

Note that option and register keys share the same
|pgfkeys| path so it is not possible to have an option and a register of the same name!

To define new registers, use the following keys:
\begin{syntax}
...
\subsection{Nodewalk keys}
\label{ref:node-walk}

A nodewalk is a sequence of steps describing a path through the tree.  Most steps
...
Besides nodewalk keys, a \meta{nodewalk} can also contain keys from the main keypath (node keys) or
...
\item First, \foRest; searches for the given \meta{keyname} in the \index{path>forest/nodewalk}
  keyspace.  If found (a long-form step or a nodewalk style), it is executed.
...
\item Otherwise, \meta{key} is executed in the \keyname{/forest} path.  This includes both
  \foRest;'s and \TikZ;'s keys.  The latter are usually forwarded to \TikZ; via \index{node
    options}.
\end{itemize}} These keys do their usual function, but within the context of the current node of the
nodewalk: |parent, s=2em,| |parent, text=red| sets the parent's |s| to 2em and the grandparent's
text color to red.  It is worth noting that these keys include \index{TeX}, which makes it
possible to execute any \TeX\ code while nodewalking.
...
Each nodewalk has an associated \emph{every-step keylist}: a keylist of node options\footnote{When
...
node is changed to the node with \index{option>id}|=0|.  Normally, such an event raises an error.
However, the full story is told by \index{nodewalk key>on invalid}.
...
not exist, the current node is set to the null node (i.e.\ node with \index{option>id}|=0|).  This
...
\indexitem{step>n}|=|$n$ the $n$th child; counting starts at $1$
...
\begin{syntax}
\indexitem{step>children}, \indexdef{step>children reversed} Visit all the children of the current
  node.
...
nodewalk.  This is true even if the final node walk reached by a fake step.
...
the final node of the embedded nodewalk.  In other words, the step made by the embedded nodewalk is
fake.
...
\indexitem{step>Nodewalk}|=|\meta{keylist: config}\meta{nodewalk}\meta{keylist: every-step}
  ...
  (and it cannot be made real by \index{real}): the last node visited by the embedded
  ...
  \begin{syntax}
    ...
    \indexitem(shared){option of=Nodewalk>history}|={|\alternative{independent,inherited,shared}|}|
      ...
      value of \keyname{every step} and \keyname{history}, respectively.
      
      \begin{tabular}{l|cccc}
        &\multicolumn{2}{c}{independent}&inherited&shared\\
        &every step&history\\\hline
        state of the outer nodewalk&\multicolumn{2}{c}{A}&A&A\\
        initial state of the inner nodewalk&given&empty&A&A\\
        &(=\meta{keylist: every-step})\\
        \dots\\
        end state of the inner nodewalk&\multicolumn{2}{c}{C}&C&C\\
        state of the outer nodewalk&\multicolumn{2}{c}{A}&A&C
      \end{tabular}
  \end{syntax}

  As shown in the table above, argument \meta{keylist: every-step} is used to initialize the
  ...
  Use \index{for step=\index{step>Nodewalk}} to return to the origin of the embedded
  nodewalk after executing it.  The return will be a fake step.  Note that this key is a bit
  exceptional.  Usually, prefix \index{for step} adds an argument (the every-step keylist).  In
  this case, however, this is not necessary, as \keyname{Nodewalk} already has the every-step
  keylist argument.
...
\indexitem{step>nodewalk}|=|\meta{nodewalk}\meta{keylist: every-step}
  ...
  \index{for step=\index{nodewalk}} is also defined.
...
\indexitem{step>group}|=|\meta{nodewalk}
  ...
  history from the outer nodewalk.
...
\indexitem(error){nodewalk key>on invalid}|={|\alternative{error,fake,step}|}|\meta{nodewalk}
  ...
  \begin{syntax}
    ...
    \item |on invalid=|\indexdef{value=on invalid>step} makes a step, either real or fake, depending on
      the environment.
  \end{syntax}
...
\indexitem{nodewalk key>options}=\meta{keylist: \foRest; node options}
...
\indexitem{node key>define long step}|=|\meta{name}\meta{options}\meta{nodewalk}
  ...
  \item\indexdef{option of=define long step>make for}|=|\meta{boolean}\hfill |true|
    ...
    Should we make a \index{prefix>for} prefix for the step? 
...
environment the conditional/loop appears in.  If the conditional/loop is a part of a nodewalk
specification, the keys are taken to be nodewalk keys (\S\ref{ref:node-walk}), otherwise node keys
(\S\ref{ref:node-options}).
...
\indexitem{loop>repeat}|=|\meta{number}\meta{keylist}
  ...
  The \meta{number} expression is evaluated using |pgfmath|.  Propagator \keyname{repeat} also works
  in nodewalks.
...
\indexitem{handler>wrap pgfmath arg}|=|\meta{macro definition}\meta{arg}
  ...
  Like \index{wrap $n$ pgfmath args} for $n=1$.
...
|__| in the pgfmath function name.  For the list of node options, see \S\ref{ref:node-options}.
...
Boolean function \indexdef{pgfmath>valid} returns true if the node's \index{option>id}$\neq0$, i.e.\ if the node is a real, valid node; see \S\ref{ref:spatial-propagators} and \S\ref{ref:node-walk}.  Boolean function \indexdef{pgfmath>invalid} is a negation of |valid|.
...
\item[v1.1 (2015/11/??)] \mbox{}
  ...
  \item The unintended and undocumented way to specify defaults using |\forestset{.style={...}}|
    ...
    \href{http://tex.stackexchange.com/questions/167972/making-a-certain-tree-style-the-default-for-forest}{Making
      a certain tree style the default for forest} at \TeX\ SE) does not work anymore. Use
    \index{default preamble}.
  \item Renamed augmented assignment operator \meta{option}|-| for prepending to \meta{toks} and
    ...
    \index{keylist-} is defined for keylist options and means ``delete key from keylist.''
  ...
  \item |compat=|\indexdef{option of=compat>1.0-name}
    ...
    package, sometimes leading to errors.  v1.1 enforces node name uniqueness. If this causes
  ...
  \item These keys have been renamed:
    ...
    \begin{tabularx}{\linewidth}{lll}
      ...
      \keyname{node walk}&\index{for step=nodewalk}\footnote{Nodewalks are much improved in v1.1, so some syntax and keys are different than in v1.0!}&\indexdef{option of=compat>1.0-nodewalk}\\
      ...
      \keyname{/tikz/fit to tree}&\index{fit to}\keyname{=}\index{tree}\footnote{The v1.0 key \keyname{/tikz/fit to tree} also set \keyname{inner sep=0}; the v1.1 key \index{fit to} does not do that.}&\indexdef{option of=compat>1.0-fittotree}\\
  ...
  \item \emph{Much} improved nodewalks, see \S\ref{ref:node-walk} and \S\ref{ref:spatial-propagators}.
  ...
  \item Introduced registers, see \S\ref{sec:registers}.
  \item Implemented handlers \index{handler>option}, \index{handler>register} and \index{process args}.
  \item Implemented several friends to \index{process keylist}, and introduced \index{processing order}.
  ...
  \item Implemented augmented assignment operator \index{keylist-} for removing keys from keylists.
  ...
  \item Added some minor keys: \index{edge path'}, \index{create'} and \index{plain content}.
